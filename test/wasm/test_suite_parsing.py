...
def do_func(fname):
    ...
    print(
        'Testing {} - "{}"'.format(
            fname, os.path.join(get_spec_suite_dir(), fname)
        )
    )
    ...
    for text in get_test_script_parts(fname):
        ...
        assert wasm_bin0 == wasm_bin1, (
            "{}: our binary parsing is broken".format(fname)
        )
        assert wasm_bin2 == wasm_bin3, (
            f"{fname}: our text/tuple paring differs"
        )
        assert wasm_bin0 == wasm_bin2, f"{fname}: our text parsing is broken"
