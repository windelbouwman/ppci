    ...
    def generate_instruction(self, ins, block):
        ...
        if isinstance(ins, ir.CJump):
            ...
            if ins.ty.is_integer:
                self.emit('{0} = correct({0}, {1}, {2})'.format(
            ...
            if isinstance(ins.value.ty, ir.BlobDataTyp):
                self.emit('write_mem({0}, {1}, {2})'.format(
                    ins.address.name,
                    ins.value.ty.size,
                    ins.value.name))
            else:
