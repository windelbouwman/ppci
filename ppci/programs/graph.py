...
from .base import (Program, SourceCodeProgram, IntermediateProgram, MachineProgram, 
                   get_program_classes)
...
def get_program_classes_by_group():
    ...
    for program in sorted(program_classes.values(), key=lambda p:p.language):
        if program in ( Program, SourceCodeProgram, IntermediateProgram, MachineProgram):
            pass
...
def get_program_classes_html():
    ...
    for column in range(3):
        ...
        for program in progs:
            ...
            t = "<a id='{}' href='{}' onmouseover='ppci_show_targets(\"{}\");' onmouseout='ppci_hide_targets();'>{}</a>"
            text += t.format(id, link, program.language, program.language) + '\n'
        columns.append('<td>{}</td>'.format(text))
    
    table_html = ("<table class='ppci-programs'>\n" +
    ...
    # Generate "graph" - the references are sorted by group so that arrows dont cross
    graphdict = {}
...
function get_rect_edge_positions(r1, r2) {
    ...
    var positions1 = [[c1[0], r1.top], [c1[0], r1.bottom], [r1.left, c1[1]], [r1.right, c1[1]]];
    for (var i=0; i<4; i++) {
        p = positions1[i];
    ...
    var positions2 = [[c2[0], r2.top], [c2[0], r2.bottom], [r2.left, c2[1]], [r2.right, c2[1]]];
    for (var i=0; i<4; i++) {
        p = positions2[i];
