    ...
    * Import classmethods like ``from_text()``.
    ...
    def __init__(self, *items, previous=None, debugdb=None):
        if not (previous is None or isinstance(previous, Program)):
            raise TypeError('Program() needs None or Program instance as "previous" argument.')
    ...
    def __repr__(self):
        return '<{} with {} items at 0x{}>'.format(self.__class__, len(self.items), hex(id(self)))
    ...
    def previous(self, which=1):
        ...
        * Program instance/class: Get the program in the compile chain that represents the given Program class.
        ...
        if which is None or which == 1:
            ...
            raise TypeError('Program.previous(which) expects int, str or Program instance/subclass.')
    ...
    # todo: force specifying in between steps and only compile if path is unique
    def to(self, language, **options):
        ...
        if chain is None:
            raise ValueError('No compile chain possible from {} to {}.'.format(self.language, language))
        program = self
...
class SourceCodeProgram(Program):
    """ Base class for source code (i.e. intended to be read and written by humans).
    """
...
class IntermediateProgram(Program):
    """ Base class for code representation that are not human readable nor machine
    executable.
    """
